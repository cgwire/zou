...
def upgrade():
    ...
    sa.Column('budget_id', sqlalchemy_utils.types.uuid.UUIDType(binary=False), default=uuid.uuid4, nullable=False),
    ...
    sa.Column('position', sqlalchemy_utils.types.choice.ChoiceType(POSITION_TYPES), nullable=True),
    sa.Column('seniority', sqlalchemy_utils.types.choice.ChoiceType(SENIORITY_TYPES), nullable=True),
    ...
    with op.batch_alter_table('budget_entry', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_budget_entry_budget_id'), ['budget_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_budget_entry_department_id'), ['department_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_budget_entry_person_id'), ['person_id'], unique=False)
