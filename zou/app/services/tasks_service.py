...
def add_asset_type_dedication(task_type_id, asset_type_id):
    """
    Add an asset type listed in database to the task type's asset types.
    """
    return _add_to_list_attr(
        task_type_id, EntityType, asset_type_id, "asset_types"
    )


def remove_asset_type_dedication(task_type_id, asset_type_id):
    """
    Remove an asset type listed in database from the task type's asset types.
    """
    return _remove_from_list_attr(
        task_type_id, EntityType, asset_type_id, "asset_types"
    )

def _add_to_list_attr(task_type_id, model_class, model_id, list_attr):
    task_type = get_task_type_raw(task_type_id)
    model = model_class.get(model_id)
    if str(model.id) not in [str(m.id) for m in getattr(task_type, list_attr)]:
        getattr(task_type, list_attr).append(model)
        return task_type
    else:
        return task_type.serialize()


def _remove_from_list_attr(task_type_id, model_class, model_id, list_attr):
    task_type = get_task_type_raw(task_type_id)
    model = model_class.get(model_id)
    try:
        getattr(task_type, list_attr).remove(model)
    except ValueError:
        pass
    return task_type