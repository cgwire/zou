...
class FileStatusTestCase(ApiDBTestCase):
    ...
    def test_get_file_status(self):
        ...
        file_status_again = self.get(f"data/file-status/{file_status['id']}")
        self.assertEqual(file_status, file_status_again)
        self.get_404(f"data/file-status/{fields.gen_uuid()}")
    ...
    def test_update_file_status(self):
        file_status = self.get_first("data/file-status")
        data = {"color": "#FFFFFF"}
        self.put(f"data/file-status/{file_status['id']}", data)
        file_status_again = self.get(f"data/file-status/{file_status['id']}")
        self.assertEqual(data["color"], file_status_again["color"])
        self.put_404(f"data/file-status/{fields.gen_uuid()}", data)

    def test_delete_file_status(self):
        file_statuss = self.get("data/file-status")
        self.assertEqual(len(file_statuss), 3)
        file_status = file_statuss[0]
        self.delete(f"data/file-status/{file_status['id']}")
        file_statuss = self.get("data/file-status")
        self.assertEqual(len(file_statuss), 2)
        self.delete_404(f"data/file-status/{fields.gen_uuid()}")
