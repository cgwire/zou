...
class CommentsServiceTestCase(ApiDBTestCase):
    ...
    def test_new_comment(self):
        ...
        self.assertEquals(self.comment["object_id"], str(self.task.id))
        self.comment = comments_service.new_comment(
            self.task.id,
            self.task_status.id,
            self.user["id"],
        ...
        self.assertEquals(self.comment["created_at"], "2024-01-23T10:00:00")
    ...
    def test_manage_status_change(self):
        ...
        self.assertEquals(task["retake_count"], 0)
        (task, status_changed) = comments_service._manage_status_change(
            retake_status, task, comment
        )
        self.assertTrue(status_changed)
        self.assertEquals(task["retake_count"], 1)
        (task, status_changed) = comments_service._manage_status_change(
    ...
    def test_reply_comment(self):
        ...
        self.assertEquals(len(comment["replies"]), 1)
        self.assertEquals(comment["replies"][0]["text"], reply_text)
        self.assertEquals(comment["replies"][0]["person_id"], self.user["id"])
        comments_service.reply_comment(
            comment["id"],
        ...
        comment = tasks_service.get_comment(comment["id"])
        self.assertEquals(len(comment["replies"]), 2)
